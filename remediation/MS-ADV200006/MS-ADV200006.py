...
# Carbon Black Cloud - MS-ADV200006 LiveResponse
# Copyright VMware 2020
# March 2020
# Version 0.1
# gfoss [at] vmware . com
#
# usage: MS-ADV200006.py [-h] [-m MACHINENAME] [-c] [-p] [-o ORGPROFILE]
# 
# optional arguments:
...
def live_response(cb, host=None, response=None):
    ...
    with device.lr_session() as lr_session:
        print ("[ * ] Uploading MS-ADV200006.ps1 to the remote host")
        lr_session.put_file(open("MS-ADV200006.ps1", "rb"), "C:\\Program Files\\Confer\\temp\\MS-ADV200006.ps1")

        if response == "patch":
            print ("[ * ] Patching the vulnerable SMBv3 configuration by disabling compression:")
            result = lr_session.create_process("powershell.exe -ExecutionPolicy Bypass -File .\\MS-ADV200006.ps1 -mitigate", wait_for_output=True, remote_output_file_name=None, working_directory="C:\\Program Files\\Confer\\temp\\", wait_timeout=30, wait_for_completion=True).decode("utf-8")
            print ("")
            print("{}".format(result))
        else:
            print ("[ * ] Checking the system for vulnerable SMBv3 configuration:")
            result = lr_session.create_process("powershell.exe -ExecutionPolicy Bypass -File .\\MS-ADV200006.ps1", wait_for_output=True, remote_output_file_name=None, working_directory="C:\\Program Files\\Confer\\temp\\", wait_timeout=30, wait_for_completion=True).decode("utf-8")
            print ("")
            print("{}".format(result))

        print ("[ * ] Removing MS-ADV200006.ps1")
        lr_session.create_process("powershell.exe del .\\MS-ADV200006.ps1", wait_for_output=False, remote_output_file_name=None, working_directory="C:\\Program Files\\Confer\\temp\\", wait_timeout=30, wait_for_completion=False)
        print ("")
...
def main():
    ...
    if args.machinename:
        ...
        print ("[ ! ] You must specify a machinename with a --machinename parameter. IE ./MS-ADV200006.py --machinename cheese")
